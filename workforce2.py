...
class App(ctk.CTk):
    ...
    def build_ui(self):
        ...
        menu_button("Dashboard", self.show_dashboard).pack(
    ...
    def show_employees(self):
        self.show_table_generic("Employee", "SELECT * FROM employees")
    ...
    def show_time_entry_form(self):
            ...
            ctk.CTkLabel(wrapper, text="Clock In (auto)", text_color=self.text_color).grid(
                row=5, column=0, pady=6, padx=5, sticky="e"
            )
            ...
            ctk.CTkLabel(wrapper, text="Clock Out (auto)", text_color=self.text_color).grid(
                row=6, column=0, pady=6, padx=5, sticky="e"
            )
            ...
            def handle_check_out():
                ...
                def update_payroll_total(emp_id: str, work_date: str, ot_hours: float):
                    ...
                    base_salary_val = float(base_salary_row[0]) if base_salary_row and base_salary_row[0] is not None else 0.0
                    ...
                    # Add new OT pay to whatever is already there; if no record, seed with base salary once.
                    new_total_salary = (existing_total or base_salary_val) + ot_pay if existing else (base_salary_val + ot_pay)
                ...
                update_payroll_total(emp_id, work_date, bf_ot_hours + af_ot_hours + bt_ot_hours)
    ...
    def show_dashboard(self):
        """Dashboard with four key metrics (employees, departments, OT hours, salary)."""
        self.clear_body()

        card = ctk.CTkFrame(self.body, fg_color=self.card_color, corner_radius=14)
        card.pack(fill="both", expand=True, padx=16, pady=16)

        header = ctk.CTkFrame(card, fg_color=self.card_color)
        header.pack(fill="x", padx=12, pady=(10, 6))
        ctk.CTkLabel(
            header,
            text="Workforce Pulse",
        ...
        metrics_frame = ctk.CTkFrame(card, fg_color=self.card_color)
        metrics_frame.pack(fill="both", expand=True, padx=10, pady=(4, 12))
        metrics_frame.grid_columnconfigure((0, 1), weight=1, uniform="metrics")
        metrics_frame.grid_rowconfigure((0, 1), weight=1, uniform="metrics")

        palette = ["#ddecff", "#ffe6f2", "#fff5d6", "#e3f7f0"]

        def metric_box(row, col, title, value, accent):
            box = ctk.CTkFrame(metrics_frame, fg_color=accent, corner_radius=14)
            box.grid(row=row, column=col, sticky="nsew", padx=8, pady=8)
            ctk.CTkLabel(box, text=title, text_color="#4b455f", font=("Helvetica", 14, "bold")).pack(anchor="center", pady=(16, 4))
            ctk.CTkLabel(box, text=value, text_color=self.text_color, font=("Helvetica", 28, "bold")).pack(anchor="center", pady=(0, 16))

        payroll_cols = {row[0] for row in self.db.fetch("SHOW COLUMNS FROM payroll")}
        has_total_salary = "total_salary" in payroll_cols
        has_total_ot = "total_ot_hours" in payroll_cols

        total_emp_row = self.db.fetch_one("SELECT COUNT(*) FROM employees")
        total_emp = total_emp_row[0] if total_emp_row else 0

        total_dept_row = self.db.fetch_one("SELECT COUNT(*) FROM department")
        total_dept = total_dept_row[0] if total_dept_row else 0

        total_salary = 0.0
        total_ot_hours = 0.0

        if has_total_salary:
            row = self.db.fetch_one("SELECT COALESCE(SUM(total_salary),0) FROM payroll")
            total_salary = float(row[0]) if row else 0.0
        if has_total_ot:
            row = self.db.fetch_one("SELECT COALESCE(SUM(total_ot_hours),0) FROM payroll")
            total_ot_hours = float(row[0]) if row else 0.0
        else:
            row = self.db.fetch_one(
                """
                SELECT COALESCE(SUM(
                    COALESCE(CAST(bf_ot AS DECIMAL(10,2)),0) +
                    COALESCE(CAST(af_ot AS DECIMAL(10,2)),0) +
                    COALESCE(CAST(bt_ot AS DECIMAL(10,2)),0)
                ),0) FROM time_records
                """
            )
            total_ot_hours = float(row[0]) if row else 0.0

        metric_box(0, 0, "Employees", f"{total_emp}", palette[0])
        metric_box(0, 1, "Departments", f"{total_dept}", palette[1])
        metric_box(1, 0, "Total OT Hours", f"{total_ot_hours:,.2f}", palette[2])
        metric_box(1, 1, "Total Salary", f"{total_salary:,.2f}", palette[3])

        # Top 10 OT employees
        list_frame = ctk.CTkFrame(card, fg_color=self.card_color)
        list_frame.pack(fill="both", expand=True, padx=12, pady=(4, 10))

        ctk.CTkLabel(
            list_frame,
            text="Top 10 Employees by OT Hours",
            font=("Helvetica", 15, "bold"),
            text_color=self.text_color,
        ).pack(anchor="w", padx=8, pady=(6, 4))

        columns = ("emp_id", "department", "ot_hours")
        tree = ttk.Treeview(list_frame, columns=columns, show="headings", height=8)
        for col in columns:
            tree.heading(col, text=col)
            tree.column(col, anchor="center", width=140 if col != "department" else 200)

        yscroll = ttk.Scrollbar(list_frame, orient="vertical", command=tree.yview)
        tree.configure(yscrollcommand=yscroll.set)
        tree.pack(side="left", fill="both", expand=True, padx=(0, 6), pady=(0, 6))
        yscroll.pack(side="right", fill="y", pady=(0, 6))

        if has_total_ot:
            top_rows = self.db.fetch(
                """
                SELECT p.emp_id,
                       COALESCE(e.department, 'Unknown') AS department,
                       ROUND(COALESCE(p.total_ot_hours,0), 2) AS ot_hours
                FROM payroll p
            ...
            top_rows = self.db.fetch(
                """
                SELECT tr.emp_id,
                       COALESCE(e.department, tr.department, 'Unknown') AS department,
                       ROUND(SUM(
                ...
                LEFT JOIN employees e ON tr.emp_id = e.emp_id
                GROUP BY tr.emp_id, department
                ORDER BY ot_hours DESC
                LIMIT 10
                """
            )
    ...
    def show_weekly_ot_over_limit(self, ot_limit: float = 20.0):
        ...
        columns = ("emp_id", "department", "total_ot_hours")
